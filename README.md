...
3. **任务完成率统一口径**（`maintenance_task_service.completion_stats`，看板、任务列表、绿地档案共用同一来源）：完成率 = 已完成任务 ÷ 有效任务；有效任务 = 全部任务 − 已取消任务。待复检、不合格记录的任务仍为「进行中」，计入分母不计入分子；已取消任务分子分母均不计入。完成率为当前时点快照，历史月份不回溯重算。
