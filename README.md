...
│   │   │   ├── code_generator.py
│   │   │   ├── task_completion.py  # 任务完成判定与完成率口径（单一来源）
...
2. **任务状态联动**（`maintenance_record_service`，判定口径集中在 `task_completion`）：
   - 任务下有养护记录后，任务自动从「待执行」进入「进行中」；
   - 存在**合格**记录且**没有待复检、不合格**记录时，任务自动置为「已完成」并写入完成时间；
   - 存在待复检或不合格记录时任务保持「进行中」：不合格须整改复检（改判合格或删除），待复检须出具结论，否则手动「标记完成」同样会被拒绝；
   - 删除养护记录后按剩余记录重新推算任务状态，避免出现「已完成却没有记录」；已取消的任务不允许补录记录。
3. **完成率单一口径**（`task_completion`，任务列表、绿地档案、总览看板共用）：完成率 = 已完成 ÷ 应完成 × 100%，**应完成 = 全部任务 − 已取消**（已取消不计入分子分母）；存在待复检/不合格记录的任务不计完成。口径只约束生效后的状态流转与实时统计，不回溯重算历史月份已落库的任务状态与完成时间。
...
```bash
cd backend
python -m pytest              # 57 个用例：接口、校验、跨模块规则、端到端流程
```

覆盖重点：绿地编号生成与唯一性、枚举与字段校验、列表过滤/排序/分页、任务状态自动流转与手动流转限制（含待复检拦截）、记录删除后的状态回退、更换金额核算、删除保护与强制删除、完成率三处口径一致性与历史状态不重算、统计聚合口径一致性、演示数据自洽性。
